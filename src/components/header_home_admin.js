  ...
  return (
    <>
      <HeaderWrapper>
        <Icons>
          <ProfileRow>
            <Avatar src={smileProfile}
            alt="프로필"
            role="button"
            onClick={() => { setLMount(true); requestAnimationFrame(() => setLOpen(true)); setROpen(false); }}
            />
            <Nick>김이화 사서</Nick>
          </ProfileRow>
          <img
        ...
        <>
          ...
          {rMount && (
            <SidebarRight role="dialog" aria-modal="true" $open={rOpen}>
              ...
              <MenuGroupRight>
                {topMenu.map(m => (
                  <MenuItemRight key={m.label} style={{ color: m.color }} onClick={() => go(m.to)}>
                    {m.label}
                  </MenuItemRight>
                ))}
              </MenuGroupRight>
