...
const formatDetail = (s) =>
  toText(s).replace(/정기휴무\(/g, "정기휴무 (").replace(/(\d{2}:\d{2})-(\d{2}:\d{2})/g, "$1 - $2");
...
const LibraryDetail = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/libraries/${libraryId}/detail/`, {
      ...
      .then(async (r) => {
        ...
        if (!r.ok) throw new Error(api?.message || `HTTP ${r.status}`);
        return api;
      })
      .then((api) => {
        if (!alive) return;
  ...
  useEffect(() => {
    ...
    const fetchSeats = async () => {
      try {
        const r = await fetch(`${BASE_URL}/seats/${libraryId}/`);
        const raw = await r.text().catch(() => "");
        let js = {};
        try { js = raw ? JSON.parse(raw) : {}; } catch {}
        if (!r.ok) throw new Error(js?.message || `HTTP ${r.status}`);
        const map = Object.fromEntries((js.seats || []).map(s => [Number(s.seat_id), String(s.status)]));
        if (alive) { setSeatStates(map); setSeatErr(""); }
      } catch (e) {
        if (alive) setSeatErr(String(e?.message || ""));
      }
    };

    fetchSeats();
    timer = setInterval(fetchSeats, 10000);
            ...
            {tab === "info" ? (
              <InfoView>
                {gallery.length > 0 && (
                  <PhotoStrip>
                    {gallery.map((src, i) => (
                      <Photo key={i}><img src={src} alt={`library-${i + 1}`} /></Photo>
                    ))}
                  </PhotoStrip>
                )}
                      ...
                      {["월", "화", "수", "목", "금", "토", "일"].map((d) => {
                        ...
                        return (
                          <li key={d}>
                            <span>{d}</span>
                            <div className="lines">
                              {lines.map((t, i) => (<em key={i}>{"\u00A0\u00A0\u00A0"}{formatDetail(t)}</em>))}
                            </div>
                          </li>
                        );
                      })}
                  ...
                  <LinkList>
                    {data.naver_map && (
                      <li>
                        ...
                        <div className="texts">
                          <a href={data.naver_map} target="_blank" rel="noreferrer">네이버 지도 — {data.name || ""}</a>
                        </div>
                      </li>
                    )}
                    {data.site && (
                      <li>
                        ...
                        <div className="texts">
                          <a href={data.site} target="_blank" rel="noreferrer">도서관 사이트 — {data.name || ""}</a>
                        </div>
                      </li>
                    )}
                  </LinkList>
                ...
                <KPIBlock>
                  <KPIRow>
                    ...
                    <LegendBar role="group" aria-label="좌석 범례">
                      <LegendItem><span>이용 가능한 좌석</span><LegendDot aria-hidden="true" /></LegendItem>
                      <LegendItem><span>이용 중인 좌석</span><LegendDot $filled aria-hidden="true" /></LegendItem>
                    </LegendBar>
                  </KPIRow>
                  <Small>(현재 좌석 수 / 전체 좌석 수)</Small>
                </KPIBlock>

                {seatErr && <Small style={{ color:"#c00" }}>좌석 갱신 오류: {seatErr}</Small>}
...
const TabBtn = styled.button`
  ...
  &:after {
    ...
    height: 2px; background: ${(p) => (p.$active ? "#000" : "transparent")};
  }
`;
...
const Photo = styled.div`
  flex: 0 0 auto; width: 184px; height: 112px; overflow: hidden; background: #f2f2f2;
  img { width: 100%; height: 100%; object-fit: cover; display: block; }
`;
const KPIBlock = styled.div`display: flex; flex-direction: column; margin: 8px 0 12px;`;
const KPIHeading = styled.div`color: #1d1d1d; font-size: 20px; font-weight: 600; line-height: 150%; margin-bottom: 4px;`;
const KPIInline = styled.div`display: flex; align-items: center; gap: 12px;`;
const Big = styled.div`color: #555; font-size: 20px; font-weight: 400; line-height: normal;`;
const Small = styled.div`color: #8e8e8e; font-size: 10px; font-weight: 300; line-height: normal; margin-top: 2px; margin-bottom: 12px;`;
...
const TimeTable = styled.ul`
  ...
  li { display: flex; gap: 0px; padding: 6px 0; align-items: flex-start; }
  span { width: 28px; color: #444444; font-size: 16px; font-weight: 400; line-height: 150%; }
  .lines { display: flex; flex-direction: column; gap: 2px; }
  em { font-style: normal; color: #444444; font-size: 16px; font-weight: 400; line-height: 150%; white-space: pre-wrap; }
`;
const SectionTitle = styled.div`color: #1d1d1d; font-size: 20px; font-weight: 600; line-height: 150%; margin-bottom: 10px;`;
const Opener = styled.div`
  display: flex; align-items: center; gap: 12px;
  .icon { width: 18px; height: 18px; object-fit: contain; }
  .texts { display: flex; align-items: center; gap: 8px; }
  .texts strong, .texts span { color: #444; font-size: 16px; font-weight: 400; line-height: 150%; }
`;
const LinkList = styled.ul`
  list-style: none; margin: 8px 0 0; padding: 0;
  li { display: flex; align-items: flex-start; gap: 10px; padding: 8px 0; }
  .icon { width: 18px; height: 18px; object-fit: contain; margin-top: 2px; }
  .texts a { color: #8e8e8e; font-size: 16px; font-weight: 400; line-height: 150%; text-decoration-line: underline; }
`;
const PhotoStrip = styled.div`
  margin: 6px -16px 16px; padding: 0 16px 4px; display: flex; gap: 7px; overflow-x: auto;
...
const LegendBar = styled.div`display: flex; flex-direction: column; align-items: flex-end; gap: 6px; margin-top: 20px;`;
const LegendItem = styled.span`display: inline-flex; align-items: center; gap: 8px; color: #8e8e8e; font-size: 12px; line-height: 150%;`;
const LegendDot = styled.i`display: inline-block; width: 10px; height: 10px; border-radius: 50%; border: 1.2px solid #bdbdbd; background: ${(p) => (p.$filled ? "#bdbdbd" : "transparent")};`;
const KPIRow = styled.div`display: flex; align-items: center; justify-content: space-between; gap: 12px;`;
